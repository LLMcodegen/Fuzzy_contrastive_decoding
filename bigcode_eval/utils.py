...
from bigcode_eval.contrast import data_handle, data_example
...
class TokenizedDataset(IterableDataset):
    ...
    def __init__(
        ...
        self.use_prompt_neg = use_prompt_neg
    ...
    def __iter__(self):
        ...
        for sample in range(self.limit_start, self.limit_start+self.n_tasks):
            ...
            if self.use_constrast_decode:
                if self.task_name == "mbpp":
                    prompt_contents = self.task.un_get_prompt(self.dataset[sample])
                    prompt_new = self.prefix + prompt_contents
                    prompts_neg.append(prompt_new)
        ...
        if self.use_constrast_decode:
            multi = ['multiple-cpp', 'multiple-cs', 'multiple-d', 'multiple-go', 'multiple-java', 'multiple-jl', 'multiple-js', 'multiple-lua', 'multiple-php', 'multiple-pl', 'multiple-py', 'multiple-r', 'multiple-rb', 'multiple-rkt', 'multiple-rs', 'multiple-scala', 'multiple-sh', 'multiple-swift', 'multiple-ts']
            
            if self.task_name == "humaneval":
                prompts_neg = data_handle(self.task_name)
            elif self.task_name == "mbpp":
                prompts_neg = list(prompts_neg)
            elif self.task_name in multi:
                name = self.task.get_name()
                prompts_neg = data_handle(name)
        ...
        if self.use_constrast_decode:
            ...
            # all_prompts.append(prompts_neg)
            outputs_neg = self.tokenizer(
                prompts_neg,
                padding="max_length",
                truncation=True,
                return_tensors="pt",
        ...
        for sample in range(self.n_tasks):
            for _ in range(self.n_copies):
                if self.has_encoder:
                    ...
                    if self.use_constrast_decode:
                        all_outputs = []
                        all_outputs.append(outputs.input_ids[sample].numpy())
                        all_outputs.append(outputs_neg.input_ids[sample].numpy())
    ...
    **gen_kwargs,
    ...
    for step, batch in tqdm(
        ...
        with torch.no_grad():
            ...
            if use_constrast_decode:
                batch_ids = batch["ids"].squeeze()
                inputs = batch_ids[:, : batch["input_len"]]
            else:
                inputs = batch["ids"][:, : batch["input_len"]]
            # inputs = batch["ids"][:, : batch["input_len"]].squeeze()
            if "ids_encoder" in batch:
                if is_wrapped:
            ...
            if use_constrast_decode:
                generated_tokens = generated_tokens[0,:].unsqueeze(0)
